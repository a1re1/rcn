//! <https://tailwindcss.com/docs/min-height>

use crate::tw_docs::demo::{el, labeled};

use crate::tw_docs::{TwExample, TwPage, TwSection};

/// <https://tailwindcss.com/docs/min-height>
pub static MIN_HEIGHT: TwPage = TwPage {
    slug: "min-height",
    title: "Min-height",
    section: TwSection::Sizing,
    description: "Utilities for setting the minimum height of an element.",
    reference: &[
        (
            "min-h-<number>",
            "min-height: calc(var(--spacing) * <number>);",
        ),
        ("min-h-<fraction>", "min-height: calc(<fraction> * 100%);"),
        ("min-h-px", "min-height: 1px;"),
        ("min-h-full", "min-height: 100%;"),
        ("min-h-screen", "min-height: 100vh;"),
        ("min-h-dvh", "min-height: 100dvh;"),
        ("min-h-dvw", "min-height: 100dvw;"),
        ("min-h-lvh", "min-height: 100lvh;"),
        ("min-h-lvw", "min-height: 100lvw;"),
        ("min-h-svw", "min-height: 100svw;"),
        ("min-h-svh", "min-height: 100svh;"),
        ("min-h-auto", "min-height: auto;"),
        ("min-h-min", "min-height: min-content;"),
        ("min-h-max", "min-height: max-content;"),
        ("min-h-fit", "min-height: fit-content;"),
        ("min-h-lh", "min-height: 1lh;"),
        (
            "min-h-(<custom-property>)",
            "min-height: var(<custom-property>);",
        ),
        ("min-h-[<value>]", "min-height: <value>;"),
    ],
    examples: &[
        TwExample {
            title: "Basic example",
            prose: &[
                "Use min-h-<number> utilities like min-h-24 and min-h-64 to set an element to a fixed minimum height based on the spacing scale:",
            ],
            snippet: r#"<div class="h-20 ...">
<div class="min-h-80 ...">min-h-80</div>
<div class="min-h-64 ...">min-h-64</div>
<div class="min-h-48 ...">min-h-48</div>
<div class="min-h-40 ...">min-h-40</div>
<div class="min-h-32 ...">min-h-32</div>
<div class="min-h-24 ...">min-h-24</div>
</div>"#,
            demo: el(
                "flex h-20 items-start gap-4",
                &[
                    labeled(
                        "min-h-80 w-16 text-xs flex items-center justify-center rounded-lg text-white bg-sky-500",
                        "min-h-80",
                    ),
                    labeled(
                        "min-h-64 w-16 text-xs flex items-center justify-center rounded-lg text-white bg-sky-500",
                        "min-h-64",
                    ),
                    labeled(
                        "min-h-48 w-16 text-xs flex items-center justify-center rounded-lg text-white bg-sky-500",
                        "min-h-48",
                    ),
                    labeled(
                        "min-h-40 w-16 text-xs flex items-center justify-center rounded-lg text-white bg-sky-500",
                        "min-h-40",
                    ),
                    labeled(
                        "min-h-32 w-16 text-xs flex items-center justify-center rounded-lg text-white bg-sky-500",
                        "min-h-32",
                    ),
                    labeled(
                        "min-h-24 w-16 text-xs flex items-center justify-center rounded-lg text-white bg-sky-500",
                        "min-h-24",
                    ),
                ],
            ),
        },
        TwExample {
            title: "Using a percentage",
            prose: &[
                "Use min-h-full or min-h-<fraction> utilities like min-h-1/2, and min-h-2/5 to give an element a percentage-based minimum height:",
            ],
            snippet: r#"<div class="min-h-full ...">min-h-full</div>
<div class="min-h-9/10 ...">min-h-9/10</div>
<div class="min-h-3/4 ...">min-h-3/4</div>
<div class="min-h-1/2 ...">min-h-1/2</div>
<div class="min-h-1/3 ...">min-h-1/3</div>"#,
            demo: el(
                "flex h-96 items-end gap-4",
                &[
                    labeled(
                        "min-h-full w-16 text-xs flex items-center justify-center rounded-lg text-white bg-indigo-500",
                        "min-h-full",
                    ),
                    labeled(
                        "min-h-9/10 w-16 text-xs flex items-center justify-center rounded-lg text-white bg-indigo-500",
                        "min-h-9/10",
                    ),
                    labeled(
                        "min-h-3/4 w-16 text-xs flex items-center justify-center rounded-lg text-white bg-indigo-500",
                        "min-h-3/4",
                    ),
                    labeled(
                        "min-h-1/2 w-16 text-xs flex items-center justify-center rounded-lg text-white bg-indigo-500",
                        "min-h-1/2",
                    ),
                    labeled(
                        "min-h-1/3 w-16 text-xs flex items-center justify-center rounded-lg text-white bg-indigo-500",
                        "min-h-1/3",
                    ),
                ],
            ),
        },
        TwExample {
            title: "Using a custom value",
            prose: &[
                "Use the min-h-[<value>] syntax to set the minimum height based on a completely custom value:",
                "For CSS variables, you can also use the min-h-(<custom-property>) syntax:",
                "This is just a shorthand for min-h-[var(<custom-property>)] that adds the var() function for you automatically.",
            ],
            snippet: r#"<div class="min-h-[220px] ...">
<!-- ... -->
</div>
<div class="min-h-(--my-min-height) ...">
<!-- ... -->
</div>"#,
            demo: el(
                "flex items-end gap-4 ",
                &[labeled(
                    "min-h-[220px] w-32 flex items-center justify-center rounded-lg text-white bg-sky-500",
                    "min-h-[220px]",
                )],
            ),
        },
        TwExample {
            title: "Responsive design",
            prose: &[
                "Prefix a min-height utility with a breakpoint variant like md: to only apply the utility at medium screen sizes and above:",
                "Learn more about using variants in the variants documentation.",
            ],
            snippet: r#"<div class="h-24 min-h-0 md:min-h-full ...">
<!-- ... -->
</div>"#,
            demo: el(
                "flex h-64 items-start rounded-lg bg-sky-300 p-2",
                &[labeled(
                    "h-24 min-h-0 md:min-h-full w-40 text-xs flex items-center justify-center rounded-lg text-white bg-sky-500",
                    "h-24 min-h-0 md:min-h-full",
                )],
            ),
        },
    ],
};
